...
class Resource:
        def __init__(self, name, resource_type):
            self.name = name
            self.type = resource_type  # Which Server Rack
...
# *****************************
mock_user_table = {
    #First user is a new test user I was using for the code when testing.
    # UUID/user_id : User
...
#use if connecting via a trusted connection
    def __init__(self):
...
#use if not connecting via a trusted connection
    def __init__(self):
        self.conn = pyodbc.connect('Driver={Driver};'
                      'Server=Server_Name;'
                      'Database=SeniorDesign;'
	...
	self.cursor.execute("
                INSERT INTO SeniorDesign.dbo.employee (employeeID, employeeFirstName, employeeLastName)
                VALUES
	...
	self.cursor.execute("
                SELECT * FROM SeniorDesign.dbo.employee
                WHERE employeeID = user_id", user_id)
                )
	for row in cursor:
    	print(row)
        return mock_user_table[user_id]

    def user_has_access_to_resource(self, user_id):
	...
	#Need to fix userID error
	self.cursor.execute("
        	SELECT userID FROM SeniorDesign.dbo.PermissionsServer(resource_id)
